...
pcons.coastal <- read_csv(file = "Coastal restoration by constituency.csv") #coastal areas to filter against
pcons.coastal <- pcons.coastal %>% filter(`All sites (count)` > 0)
...
#GA.2 <- GA.2 %>% filter(pcon19cd %in% pcons.coastal$pcon19cd)
rm(pcons.coastal)
...
plot <- leaflet(height = 1600,options= leafletOptions(padding = 100, zoomSnap = 0.25, zoomDelta = 0.3, zoomControl = F)) %>%
  ...
  addCircleMarkers(data = coastal.resto[coastal.resto$Priority == "N",],
                   color = "#315C73",
                   opacity = 1,
                   fillOpacity = 1,
                   radius = 3, 
                   weight = 1.5,
                   popup = paste0(coastal.resto$Site_Name)) %>% 
  
  addCircleMarkers(data = coastal.resto[coastal.resto$Priority == "Y",],
                   color = "#B3C8D1",
                   opacity = 1,
                   fillOpacity = 1,
                   radius = 3, 
  ...
  addLegendCustom(colors = c("#315C73","#B3C8D1"), 
                  labels = c("Other restoration areas", "Priority  restoration areas"),

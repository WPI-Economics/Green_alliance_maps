...
Coastal.resto <- readRDS(file = "Dashboard/Coastal.resto.RDS")
...
# plot <- leaflet(GA.2,height = 1600,options= leafletOptions(padding = 100, zoomSnap = 0.25, zoomDelta = 0.3, zoomControl = T, minZoom = 5.3)) %>%
#   setView(lng =  -3.13,
...
# 
# 
# addLegendCustom(colors = c("#EE2E24"),
#                 labels = c("Coastal restoration sites"),
# 
...
plot <- leaflet(height = 1600,options= leafletOptions(padding = 100, zoomSnap = 0.25, zoomDelta = 0.3, zoomControl = F)) %>%
  ...
  addCircleMarkers(data = Coastal.resto,
                   color = "#EE2E24",
                   opacity = 1,
                   fillOpacity = 1,
                   radius = 3,
